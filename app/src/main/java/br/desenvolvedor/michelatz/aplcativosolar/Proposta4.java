...
public class Proposta4 extends AppCompatActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        txtPorcentagem = (TextView) findViewById(R.id.txtPorcentagem);

        progressBar.setScaleY(10f);
        progressBar.setProgress(70);
        txtPorcentagem.setText("70%");
        progressBar.getProgressDrawable().setColorFilter(R.drawable.botao, PorterDuff.Mode.SRC_IN);
        //progressBar.setForeground(R.drawable.botao);
        //progressBar.setBackground(R.drawable.botao);
        //progressBar.setBackground(new Color());
    ...
    private void inflaListaConsumoMes() {
        ...
        Cursor linhas = db.rawQuery("SELECT *  FROM " + TABELADADOSTELHADO+ " WHERE IDPROPOSTA = "+ idProposta +";", null);
        int contador = 0;
        if (linhas.moveToFirst()) {
            do {
                ...
                String texto = " Orientação - " +linhas.getString(4); //valores
    ...
    public void editaItem(View v) {
        ...
        Intent intent = new Intent(this, AdicionaDados.class);
    ...
    private void deletarMensagem(final String idMens) {
        ...
        db = openOrCreateDatabase(BANCO, Context.MODE_PRIVATE, null);
        db.execSQL("DELETE FROM " + TABELADADOSTELHADO + " WHERE ID = " + idExcluido + "");
        db.close();
    ...
    public void finalizar(View v) {
        db = openOrCreateDatabase(BANCO, Context.MODE_PRIVATE, null);
        Cursor linhas = db.rawQuery("SELECT ID FROM " + TABELADADOSTELHADO + " WHERE IDPROPOSTA = "+ idProposta +";", null);

        if (linhas.moveToFirst() && linhas.getCount() > 0) {
                Intent it = new Intent(this, VisualizaPropostaCompleta.class);
                startActivity(it);
                finish();

        } else{
            Toast.makeText(getApplicationContext(), "Preencha pelo menos 1 Dado!!!", Toast.LENGTH_SHORT).show();
        }
    }
